...
describe('Return the correct amount earned by a dart landing in a given point in the target problem', () => {
  ...
  test('Expect an error if either the X or Y coordinate is not a number', () => {
    expect(() => solve(true, 4)).toThrow(new Error('Both coordinates must be numbers!'));
    expect(() => solve(4 ,'3')).toThrow(new Error('Both coordinates must be numbers!'));
  });
});
